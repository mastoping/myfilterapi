...
export default async function handler(req, res) {
    ...
    try {
        ...
        const response = await fetch(
            `http://localhost:20018/listBYU?username=${username}&payment_method=${payment_method}&to=${to}&productSubCategory=${productSubCategory}&json=1`
        );
        ...
        const formattedResult = data.res.map((item) => {
            return `PID: ${item.productId} (${item.productName} Hrg ${item.price})`;
        });
