...
export default async function handler(req, res) {
    ...
    const { username, payment_method, to } = req.query;

    // Validasi parameter yang diperlukan
    if (!username || !payment_method || !to ) {
    ...
    try {
        ...
        const response = await fetch(
            `http://localhost:20018/listTerbaik?username=${username}&category=VOICE_SMS&payment_method=${payment_method}&to=${to}&json=1`
        );
